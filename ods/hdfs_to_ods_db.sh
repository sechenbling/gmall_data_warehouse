...
# 将业务数据从HDFS移动到指定的Hive表路径下
# 使用方法： hdfs_to_ods_db.sh 表名(可选all) 时间日期
# 示例 hdfs_to_ods_log.sh all 2020-06-14
# 定义变量方便修改
...
load_data(){
    ...
    for i in $*; do
        echo "${i}进行同步操作"
        #判断路径是否存在
        # ${i:4}指的是${i}截取第四个字符之后的所有字符串（即去除 ods_ 前缀）
        hadoop fs -test -e /origin_data/$APP/db/${i:4}/$do_date

        #路径存在方可装载数据 $?为获取上一条命令数据 0为存在 1为不存在
        if [[ $? = 0 ]]; then
